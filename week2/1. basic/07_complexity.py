    ...
    for i in range(n):
        for j in range(i+1 , n):
            if(nums[i] == nums[j]): 
                ...
                #     if(duplicates[k] == nums[i]): isDuplicated = True
                # if(isDuplicated is False): duplicates.append(nums[i])
                if nums[i] not in duplicates: duplicates.append(nums[i]) # Collection의 in 연산자 알게 되었음
                else: continue
...
def find_duplicates_sorting(nums):
    ...
    for i in range(len(nums)-1):
        if(nums[i] == nums[i+1]):
            ...
            #     if(duplicates[j] == nums[i]): isDuplicated = True
            # if isDuplicated is False : duplicates.append(nums[i])
